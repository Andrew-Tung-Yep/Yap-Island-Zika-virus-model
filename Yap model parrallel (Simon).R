...
setwd(nl.path)
drop.path <- "/home/andrew/Yap geographical sir (R) mod.nlogo" # The model
folderdir <- "/home/andrew/RNetlogo" # Where you want to save the results file
tabname <- "zika_parallel.csv" # Name of results file to be written
#```
...
#```{r}
parnames <- c("Bm","Bh","mospop","mosspread","popspread","Ah","Ch","distm","disth","Am","mosdeath")
npar <- length(parnames)
lwr <- c(0,0,12,2,5,0.132,0.174,0.26,0.5,0.083,0.09)
upr <- c(0.5,0.5,107,9,20,0.227,0.908,0.86,1,0.25,0.341)
data.frame(parnames,lwr,upr)
nparams <- length(lwr)
parmatrix <- as.data.frame(matrix(0.0,nsims,nparams))
for(i in c(1,2,4,6:11)){
...
write.csv(parmatrix,paste(folderdir,"zika_parallel_parmatrix.csv",sep=""),row.names=F)
#````